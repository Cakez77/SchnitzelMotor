...
GLFWwindow* window;
const char* window_name = "Schnitzel Motor";
...
void cursor_position_callback(GLFWwindow* window, double xpos, double ypos)
{
  ...
  // scale up the mouse position so that it matches the screen size
...
void mouseButtonCallback(GLFWwindow* window, int button, int action, int mods)
{
  ...
  input->prevMousePos = input->mousePos;
  input->mousePos = {(int)(win_x * scaleX), (int)(win_y * scaleY)};
  input->relMouse = input->mousePos - input->prevMousePos;
...
{
  running = false;  // Exit the game loop and close the window
}
